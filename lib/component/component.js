...
function Component(domNode, surrogateRoot, parent, famousConnector, runtimeConfiguration, cb) {
    // Identifying data
    ...
    this.famousConnector = famousConnector;
    ...
    var famousNodeConstructor = famousNodeConstructorName ?
                                    ...
                                    null;
    this.famousNode = this.famousConnector.addChild(parent.famousNode, famousNodeConstructor);
    this.states = new States(this.famousNode, this.definition.states, this.famousConnector);
    ...
    this.events = new Events(this.definition.events, this.name, this.dependencies, this.getRootNode(), this.famousConnector);
...
Component.prototype._createExpandedBlueprintObserver = function _createExpandedBlueprintObserver(expandedBlueprint) {
    ...
    this._observer = new MutationObserver(function(mutations) {
        ...
        // Initialize events on created components
...
Component.prototype._updateChildren = function _updateChildren(childrenRoot) {
    ...
    this.tree.eachChild(function(node) {
        if (VirtualDOM.isValidHTMLElement(node)) {
            ...
            return new Component(baseNode, node, self, this.famousConnector);
        }
    });
...
Component.prototype._attachDOMWrapper = function _attachDOMWrapper(domNodes) {
    var wrapperNode = this.famousConnector.addChild(this.famousNode);
    var content = '';
    for (var i = 0; i < domNodes.length; i++) {
        content += domNodes[i].outerHTML;
    }
    var famousDomElement = this.famousConnector.attachDOMElement(wrapperNode, content);
...
Component.prototype._processControlFlowMessages = function _processControlFlowMessages(behavior) {
    ...
    for (var i = 0; i < nodes.length; i++) {
        result = this._processControlFlowMessage(nodes[i], expandedBlueprint);
...
Component.prototype._processControlFlowMessage = function _processControlFlowMessage(node, progenitorExpandedBlueprint) {
    ...
    if (info) {
        ...
        if (info.message === CREATE_KEY) {
            ...
            return new Component(baseNode, newNode, DataStore.getComponent(parentUID), this.famousConnector);
        }
...
Component.executeComponent = function executeComponent(frameworkInstance, name, tag, selector, configuration, cb) {
    ...
    return new Component(baseNode, null, {
        tree: topLevelTree,
        famousNode: frameworkInstance.famousConnector.createRoot(selector),
        rootSelector: selector
    }, frameworkInstance.famousConnector, configuration, cb);
};
