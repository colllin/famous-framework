...
function Events(eventGroups, name, dependencies, rootNode, famousConnector) {
    EventHandler.apply(this);
    this.name = name;
    this.famousConnector = famousConnector;
    this.injector = new Injector(this.famousConnector);
...
Events.prototype.setupProxyEvent = function setupProxyEvent(frameworkEvent, targets, uid) {
    ...
    for (var i = 0; i < targets.length; i++) {
        ...
        var getArgs = this.injector.getArgs.bind(this.injector);
        ...
        if (specialIndicies) {
            ...
            payload.listener = function(boundComponent, eventPayload) {
                listenerArgs = getArgs(frameworkEvent.params, null, uid);
        ...
        triggerArgs = getArgs(triggerParams, payload, targetUID);
    ...
    for (var i = 0; i < targets.length; i++) {
        ...
        component.events.dispatcher.on(eventName, function(component, customEvent) {
            ...
            for (i = 0; i < eventsToFire.length; i++) {
                ...
                args = getArgs(event.params, customEvent.detail, uid);
                if (specialIndicies) {
...
Events.prototype.triggerDirectEventHandler = function triggerDirectEventHandler(name, payload, uid) {
    ...
    var handlerArgs = this.injector.getArgs(handlerParams, payload, uid);
...
Events.prototype._executeEvent = function _executeEvent(event, payload, uid) {
    var args = this.injector.getArgs(event.params, payload, uid);
    event.action.apply(null, args);
};
