...
function Injector(famousConnector) {
    this.famousConnector = famousConnector;
}

Injector.prototype.getArgs = function(paramNames, payload, uid) {
    ...
    for (var i = 0; i < paramNames.length; i++) {
        switch (paramNames[i]) {
            ...
            case DOM_ELEMENT_KEY: args.push(this.famousConnector.decorateComponent(component, 'DOMElement')); break;
            case MESH_KEY: args.push(this.famousConnector.decorateComponent(component, 'Mesh')); break;
            case EVENT_KEY: args.push(null); break; // `$event` gets overwritten with event object if one exists
            case GESTURE_HANDLER_KEY: args.push(this.famousConnector.GestureHandler); break;
            default:
